...
from app.engine import GameState, Monster, global_game_state
from app.tools import select_character as select_character_tool
...
# --- GM tool-layer steps (exercise app/tools.py against the shared singleton) ---


@given("the game engine state is reset")
def step_impl(context):
    # The tools operate on the module-level singleton, so reset it in place.
...
@when(
    'the GM calls the select_character tool with class "{char_class}" and name "{name}"'
)
def step_impl(context, char_class, name):
    context.tool_result = select_character_tool(char_class, name)


@then("the tool reports success")
def step_impl(context):
    assert context.tool_result["status"] == "success"


@then(
    'the global party has a "{p_class}" player named "{p_name}" and companion "{c_name}" the "{c_class}"'
)
def step_impl(context, p_class, p_name, c_name, c_class):
    assert global_game_state.player.char_class == p_class
    assert global_game_state.player.name == p_name
    assert global_game_state.companion.name == c_name
    assert global_game_state.companion.char_class == c_class


@then("the tool reports an error and no party is created")
def step_impl(context):
    assert context.tool_result["status"] == "error"
...
@then(
    'the player character is a "{class_name}" named "{player_name}" with health {hp:d} and mana {mana:d}'
)
...
@then(
    'the companion character is a "{class_name}" named "{companion_name}" with health {hp:d} and level {lvl:d}'
)
...
@then(
    'the paths have different difficulties: "left" is "{diff1}", "forward" is "{diff2}", "right" is "{diff3}"'
)
...
def step_impl(context, monster_name, hp):
    context.game_state.active_monster = Monster(
        monster_name, hp, attack=5, xp_reward=20, gold_reward=10
    )
...
def step_impl(context, spell_name, target):
    ...
    wizard = (
        ...
        if context.game_state.player.char_class == "Wizard"
        else context.game_state.companion
    )
...
def step_impl(context):
    is_over, _msg = context.game_state.check_combat_end()
    assert is_over
...
def step_impl(context, hp):
    fighter = (
        context.game_state.player
        if context.game_state.player.char_class == "Fighter"
        else context.game_state.companion
    )
    fighter.hp = hp
