...
That loop caught real behavioral bugs. The first graded run scored **4.125/5**: the judge flagged that the GM asked for confirmation instead of executing multi-step commands ("create a Wizard, then take the left path…") and that it rigidly presented routes after character creation even when the player had asked for something else. Two targeted instruction changes later, the same dataset scored **4.875/5** (7×5.0, 1×4.0, zero errors). Evals-as-regression-tests, working as advertised.
