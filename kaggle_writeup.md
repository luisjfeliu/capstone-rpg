...
**MCP server.** The entire engine is additionally exposed as a Model Context Protocol server (`app/mcp_server.py`, FastMCP over stdio) registering the same nine tool functions — zero logic duplication. Setting `RPG_USE_MCP=1` switches the GM from in-process tools to consuming the engine through ADK's `MCPToolset`, and an integration test drives the server over the real stdio protocol. The dungeon is now a service any MCP client can play.
...
That loop caught real behavioral bugs. The first graded run scored **4.125/5**: the judge flagged that the GM asked for confirmation instead of executing multi-step commands ("create a Wizard, then take the left path…") and that it rigidly presented routes after character creation even when the player had asked for something else. Two targeted instruction changes raised the same dataset to **4.875/5**. Playtesting later exposed the GM narrating hallucinated HP numbers that contradicted the live status panel; a "never invent numbers — describe condition qualitatively" rule fixed it and brought the dataset to **5.0/5** (8/8 cases, zero errors). Evals-as-regression-tests, working as advertised.
