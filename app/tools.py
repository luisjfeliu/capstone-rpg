...
def get_game_status() -> dict:
    ...
    g = global_game_state
    if not g.player:
        return {"status": "error", "message": "Game not started. No character selected."}
...
def select_route(direction: str) -> dict:
    ...
    if direction not in ["left", "forward", "right"]:
        return {"status": "error", "message": "Direction must be 'left', 'forward', or 'right'."}
    try:
        g.choose_route(direction)
        return {"status": "success", "message": f"Route selected: {direction}. Moving forward!"}
...
def enter_room() -> dict:
    ...
    if not g.selected_route:
        return {"status": "error", "message": "No route selected. Select a route first."}
        
    if g.current_room_index >= g.selected_route.length:
        return {"status": "complete", "message": "You have cleared this route. You can advance to the next level."}
...
def execute_weapon_attack(attacker_name: str, target: str) -> dict:
    ...
    if not attacker:
        return {"status": "error", "message": f"Character {attacker_name} not found in party."}
        
    if not attacker.is_alive():
        return {"status": "error", "message": f"{attacker.name} is unconscious and cannot attack."}
...
def execute_cast_spell(attacker_name: str, spell_name: str, target: str) -> dict:
    ...
    if not attacker.is_alive():
        return {"status": "error", "message": f"{attacker.name} is unconscious and cannot cast spells."}
