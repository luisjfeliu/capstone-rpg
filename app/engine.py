...
class Monster:
    def __init__(self, name: str, hp: int, attack: int, xp_reward: int, gold_reward: int):
        self.name = name
        self.max_hp = hp
        self.hp = hp
        self.attack = attack
...
class Route:
    def __init__(self, direction: str, length: int, difficulty: str, monster_types: List[str], treasure: str, xp_mult: float):
...
class GameState:
    ...
    def select_character(self, player_class: str, player_name: str) -> Tuple[Character, Character]:
    ...
    def generate_level_routes(self):
        ...
        for direction, length, diff in routes_data:
            monsters = [random.choice(monsters_by_diff[diff]) for _ in range(length - 1)]
            ...
            # Treasure logic
            gold_amount = level * (20 if diff == "easy" else 40 if diff == "medium" else 80)
            item = "Health Potion" if diff == "easy" else "Mana Potion" if diff == "medium" else "Elixir of Life"
    ...
    def cast_spell(self, caster: Character, spell_name: str, target: str) -> str:
        ...
        if spell_name == "Fireball":
            ...
            # Find heal target
            heal_target = self.player if target.lower() == self.player.name.lower() or target.lower() == "player" else self.companion
            ...
            shield_target = self.player if target.lower() == self.player.name.lower() or target.lower() == "player" else self.companion
    ...
    def check_combat_end(self) -> Tuple[bool, str]:
        ...
        if not self.active_monster.is_alive():
            ...
            if self.selected_route and self.current_room_index >= self.selected_route.length:
                # Add route rewards
    ...
    def next_level(self) -> Tuple[bool, str]:
        ...
        if self.current_level >= 7:
            self.game_won = True
            self.game_over = True
            return True, "You step through the portal on level 7 and cross into another dimension! You have won the game!"
        ...
        return False, f"You advance to level {self.current_level}. The party is fully healed and rested."
